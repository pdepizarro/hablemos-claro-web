    ...
    <form aria-label="Formulario de donación" noValidate onSubmit={handleSubmit} className="space-y-6">
      <fieldset>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          <button
            ...
            className={`rounded-hc-lg border px-3 py-3 text-left transition-colors sm:px-4 sm:py-4 ${
              mode === "one_time"
          ...
          >
            <p className="font-heading text-lg font-bold leading-tight sm:text-xl">Donación puntual</p>
            <p className="mt-1 text-xs text-hc-muted sm:text-sm">Pago único</p>
          </button>
          <button
            type="button"
          ...
          >
            <p className="font-heading text-lg font-bold leading-tight sm:text-xl">Donación mensual</p>
            <p className="mt-1 text-xs text-hc-muted sm:text-sm">Suscripción recurrente</p>
          </button>
        </div>
      </fieldset>
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-3 sm:gap-3 lg:grid-cols-5">
          {DONATION_PRESET_AMOUNTS.map((amount) => (
            <label
              key={amount}
              className={[
                "flex cursor-pointer items-center justify-center rounded-hc-lg border px-3 py-2.5 text-sm font-semibold transition sm:px-4 sm:py-3 sm:text-base",
                presetAmount === amount
        ...
        <div className="mt-3 sm:mt-4">
          <label htmlFor="custom-amount" className="mb-1.5 block text-sm font-semibold text-hc-text sm:mb-2">
          ...
          <div className="flex overflow-hidden rounded-hc-lg border border-white/20 bg-black/40 focus-within:border-hc-yellow">
            ...
            <input
              ...
              className="flex-1 bg-transparent px-3 py-2.5 text-sm text-hc-text placeholder-hc-muted focus:outline-none sm:py-3 sm:text-base"
            />
          </div>
        </div>
      </fieldset>
      ...
      {errorMessage && (
        <p className="rounded-hc-lg border border-hc-red/35 bg-hc-red/10 px-3 py-2.5 text-sm text-hc-red sm:px-4 sm:py-3">
          {errorMessage}
        </p>
      )}
        ...
        >
          {status === "loading"
            ...
            : resolvedAmount === null
              ...
              : mode === "monthly"
                ? `Donar ${formatEuro(resolvedAmount)}/mes`
                : `Donar ${formatEuro(resolvedAmount)} ahora`}
        </button>

        <p className="mt-2 text-center text-xs text-hc-muted sm:mt-3 sm:text-sm">
          {mode === "monthly"
